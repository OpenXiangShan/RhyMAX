...
# ==== 转换为 RF_Data (C) ====
RF_Data_C = [[0] * (numM * numN * m) for _ in range(n // 4)]
for i in range(numM):
    for j in range(numN):
        for p in range(n // 4):
            for q in range(m):
                hex_str = ""
                for r in range(4):
                    elem = C0[i * m + q][j * n + p * 4 + r]
                    hex_str += f"{elem & 0xFFFFFFFF:08x}"
                RF_Data_C[p][i * numN * m + j * m + q] = hex_str
...
with open(l2_file, "w") as f:
    ...
    for i in range(row):
        ...
        for j in range(0, stride, 16):
            ...
            f.write(f"    {line},\n")
        f.write("  )\n\n")
    all_lines = " ++ ".join(f"L2_Line{i}" for i in range(row))
...
with open(rf_file, "w") as f:
    ...
    write_matrix("A", RF_Data_A)
    write_matrix("B", RF_Data_B)
    write_matrix("C", RF_Data_C)
